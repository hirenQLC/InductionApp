...
open class LNZSnapToCenterCollectionViewLayout: UICollectionViewLayout, FocusedContaining {
    ...
    override open var collectionViewContentSize: CGSize {
        ...
        let width: CGFloat = sectionInsetLeft + sectionInsetRight - interitemSpacing + (itemSize.width + interitemSpacing) * CGFloat(itemCount ?? 0)
        let headHeight = (headerHeight ?? 0.0)
        let footHeight = (footerHeight ?? 0.0)
        let height: CGFloat = headHeight + sectionInsetTop + sectionInsetBottom + itemSize.height + footHeight
    ...
    internal func items(in rect: CGRect) -> [(index:IndexPath, frame: CGRect)] {
        ...
        for iVar in firstIndex...lastIndex {
            let indexPath = IndexPath(item: iVar, section: 0)
    ...
    internal func frameForItem(at indexPath: IndexPath) -> CGRect {
        let xAxis = sectionInsetLeft + (itemSize.width + interitemSpacing) * CGFloat(indexPath.item)
        let yAxis = (headerHeight ?? 0) + sectionInsetTop
        return CGRect(origin: CGPoint(x: xAxis, y: yAxis), size: itemSize)
    }

...
open class LNZInfiniteCollectionViewLayout: LNZSnapToCenterCollectionViewLayout {
    ...
    private func frameForCycle(at index: Int) -> CGRect {
        ...
        let xAxis = cycleStart + cycleSize.width * CGFloat(index)
        return CGRect(x: xAxis, y: 0, width: cycleSize.width, height: cycleSize.height)
    }
    override internal func items(in rect: CGRect) -> [(index:IndexPath, frame: CGRect)] {
        ...
        for iVar in iFirstCycle...iLastCycle {
            let cycleFrame = frameForCycle(at: iVar)
            ...
            if firstIndex <= lastIndex { /// added to avoid crash
                for jVar in firstIndex...lastIndex {
                    if jVar >= 0 {
                        let xAxis = cycleFrame.origin.x + (itemSize.width + interitemSpacing) * CGFloat(jVar)
                        let yAxis = sectionInsetTop + (headerHeight ?? 0)
                        indexPaths.append((
                            IndexPath(item: jVar, section: 0),
                            CGRect(origin: CGPoint(x: xAxis, y: yAxis), size: itemSize)
                        ))
                    }
                }
            }
    ...
    open override func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
        ...
        for item in items(in: collectionView.bounds) {
        ...
        rectAttributes.append(contentsOf: attributesForHeaderAndFooter())
    ...
    open override func layoutAttributesForItem(at indexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
        ...
        if let item = itemsOnScreen.first(where: { $0.index == indexPath })  {
            ...
            let xAxis = cycleFrame.origin.x + (itemSize.width + interitemSpacing) * CGFloat(indexPath.item)
            let yAxis = sectionInsetTop
            frame = CGRect(origin: CGPoint(x: xAxis, y: yAxis), size: itemSize)
        }
